...
That's what this next chunk is. We are loading some libraries and importing our data again.
...
Usually when we build a chart, we would keep adding our layers into a **SINGLE** code chunk, rerunning the code with each addition to make sure the changes work the way we want. In this exercise, we are creating new code chunks so you can see the difference between each layer as we add it.
...
Now that you have data, plot that into a column chart:
...
So far, we've been plotting one type of thing. But what if we want to see how the popularity of a name has changed over time? We can use a line chart to show that.
...
In the second step we can use the `pull()` function to pull out just the names from the data set. We save that, too, because it's what we'll use to filter our data.
...
top_5f_years |> ggplot(aes(x = year, y = prop, color = name)) +
  ...
  scale_x_continuous(breaks = seq(1880,2023,10)) + # adjust x grid lines
  theme_minimal() +
  labs(
...
> I NEED TO SUPPLY AN IDEA FOR THIS?
...
Let's walk through an example of this by ploting the popularity of the top 20 names over time.
...
top20_names_data |> ggplot(aes(x = year, y = prop, color = name)) +
  ...
  scale_x_continuous(breaks = seq(1900,2023,40)) +
  theme_minimal() +
