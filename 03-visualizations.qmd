...
We need all this stuff to get started.
...
babynames <-  read_rds("data/babynames_ranked.rds")
```
...
bar_chart_data %>% ggplot(aes(x = reorder(name,desc(n)), y = n)) +
  geom_col(fill = "cadetblue", alpha = .8) +
  scale_y_continuous(labels = label_comma()) +
  labs(title = "The 10 most common baby names in 2023", subtitle = "None of these are Heaven spelled backwards.") +
  theme_minimal() +
...
bar_chart_data %>% ggplot(aes(x = reorder(name,desc(n)), y = n)) +
  geom_col(fill = "cadetblue", alpha = .8) +
  scale_y_continuous(labels = label_comma()) +
  labs(title = "The 10 most common baby names in 2023", subtitle = "None of these are Heaven spelled backwards.", caption = "Source: Social Security Administration\nby Olga Pierce") +
  theme_minimal() +
  theme(axis.title = element_blank(),
...
```{r}
bar_chart_data %>% ggplot(aes(x = reorder(name,desc(n)), y = n, fill = sex)) +
  geom_col(alpha = .8) +
  scale_y_continuous(labels = label_comma()) +
  labs(title = "The 10 most common baby names in 2023", subtitle = "None of these are Heaven spelled backwards.", caption = "Source: Social Security Administration\nby Olga Pierce") +
  theme_minimal() +
...
```{r}
line_chart_data <- babynames %>% filter(sex == "F" & name %in% c("Olive", "Betty", "Madison", "Nevaeh", "Stephanie"))
...
line_chart_data %>% ggplot(aes(x = year, y = prop, color = name)) +
  ...
  scale_x_continuous(breaks = seq(1900,2023,10)) +
  theme_minimal() +
  theme(axis.title = element_blank()) +
...
```{r}
top_names <- babynames %>% group_by(name) %>% summarize(total = sum(n)) %>% arrange(desc(total)) %>% top_n(20,total)

facet_names <- inner_join(babynames, top_names)
```
